...
export default function BigCard({ src, title, author, serie, tome, note, sku }) {
    ...
    return(
        <Link href={`/book/${sku}`}>
            <a className={`relative flex items-end rounded-lg text-gray-600 hover:text-black rounded-lg min-h-[170px] min-w-[240px] ${styles.bigCardHover}`}>

                <img src={src} alt={`Une couverture de livre`} className={`flex absolute bottom-4 left-4 self-start justify-self-start ${styles.image} rounded-lg`}/>
